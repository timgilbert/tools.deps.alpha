...
(ns clojure.tools.deps.alpha.libmap
...
(defn load-lib-map
  ...
  []
  (or (read-libmap) (read-libfile) (read-cp)))
...
(defn lib-map
  ...
  []
  (or @the-map (reset! the-map (load-lib-map))))
...
(comment
  (System/setProperty "clojure.libfile" "/Users/alex/.clojure/.cpcache/3334154579.libs")
  (def s (pr-str (read-libfile)))
  (System/setProperty "clojure.libmap" s)
  (def s2 (pr-str (read-libmap)))
  (= s s2)

  (clojure.pprint/pprint (read-cp))
  )
